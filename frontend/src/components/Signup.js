...
export default function Signup() {
  ...
  const validateForm = () => {
    if (!formData.email.endsWith('@cooper.edu')) {
      setMessage("Please use a @cooper.edu email."); return false;
    }
    if (formData.password.length < 9) {
      setMessage("Password must be at least 9 characters."); return false;
    }
    const y = parseInt(formData.year, 10);
    if (isNaN(y) || y < 2020 || y > 2050) {
      setMessage("Year must be between 2020 and 2050."); return false;
    }
    if (!allowedMajors.includes(formData.major)) {
      setMessage("Please select a valid major."); return false;
    }
    if (!agreeToShare) {
      setMessage("You must agree to share your email with future matches."); return false;
    }
    return true;
  };
